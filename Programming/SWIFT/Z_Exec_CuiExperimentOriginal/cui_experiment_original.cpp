...
typedef struct GaussLegendreAW{
    int numgrid; // # of nodes
    double* u; // nodes
    double* w; // weights
} GLAW;
static GLAW gauss_legendre_aw = {64, x64, w64};
//static GLAW gauss_legendre_aw = {96, x96, w96};
//static GLAW gauss_legendre_aw = {128, x128, w128};
...
HestonIntegrandsMN GetHestonIntegrandsMN(double u, double a, double b, double c, double rho, double v0,
                                         double K, double T, double S, double r) {
    HestonIntegrandsMN MNbas;
...
void GetHestonPrice(double *p, double *x, int /*m*/, int n_observations, void* data_ptr_void)
{
    ...
    int NumGrids = gauss_legendre_aw.numgrid;
    NumGrids = (NumGrids + 1UL) >> 1UL;
    double *u = gauss_legendre_aw.u;
    double *w = gauss_legendre_aw.w;

    for (l=0; l < n_observations; ++l) {
        double K = data_ptr->K[l];
        double T = data_ptr->T[l];
        double disc = exp(-r*T);
        ...
        for (int j=0; j< NumGrids; j++) {
            ...
            HestonIntegrandsMN MN = GetHestonIntegrandsMN(u[j], kappa, v_bar, sigma, rho, v0, K, T, S, r);
...
int main1() {
    ...
    int n_observations = 1; // # of observations (consistent with the struct MarketParameters)
    ...
    market_parameters.S = 80.0;
    market_parameters.r = 0.03;

    //// END INIT MARKET PARAMETERS
    // you may set up your optimal model parameters here:
    // set optimal model parameters  |  Meaning
    double kappa = 1.0;           // |  mean reversion rate
    double v_bar = 0.05;          // |  long term variance
    double sigma = 0.2;           // |  variance of volatility
    double rho = -0.7;            // |  correlation between spot and volatility
...
int main2() {
    ...
    double multiplier = 1;
    double expiries[] = {
        multiplier * 0.119047619047619, multiplier * 0.119047619047619, multiplier * 0.119047619047619, multiplier * 0.119047619047619, multiplier * 0.119047619047619,
        multiplier * 0.119047619047619, multiplier * 0.119047619047619, multiplier * 0.119047619047619, multiplier * 0.119047619047619, multiplier * 0.119047619047619,
        multiplier * 0.119047619047619, multiplier * 0.119047619047619, multiplier * 0.119047619047619, multiplier * 0.119047619047619, multiplier * 0.119047619047619,
        multiplier * 0.119047619047619, multiplier * 0.119047619047619, multiplier * 0.119047619047619, multiplier * 0.119047619047619, multiplier * 0.119047619047619,
        multiplier * 0.119047619047619, multiplier * 0.119047619047619, multiplier * 0.119047619047619, multiplier * 0.119047619047619, multiplier * 0.119047619047619,
        ...
        multiplier * 0.119047619047619, multiplier * 0.119047619047619, multiplier * 0.119047619047619, multiplier * 0.119047619047619, multiplier * 0.119047619047619,
        multiplier * 0.119047619047619, multiplier * 0.119047619047619, multiplier * 0.119047619047619, multiplier * 0.119047619047619, multiplier * 0.119047619047619
//        multiplier * 0.238095238095238, multiplier * 0.238095238095238, multiplier * 0.238095238095238, multiplier * 0.238095238095238, multiplier * 0.238095238095238,
//        multiplier * 0.357142857142857, multiplier * 0.357142857142857, multiplier * 0.357142857142857, multiplier * 0.357142857142857, multiplier * 0.357142857142857,
//        multiplier * 0.476190476190476, multiplier * 0.476190476190476, multiplier * 0.476190476190476, multiplier * 0.476190476190476, multiplier * 0.476190476190476,
//        multiplier * 0.595238095238095, multiplier * 0.595238095238095, multiplier * 0.595238095238095, multiplier * 0.595238095238095, multiplier * 0.595238095238095,
//        multiplier * 0.714285714285714, multiplier * 0.714285714285714, multiplier * 0.714285714285714, multiplier * 0.714285714285714, multiplier * 0.714285714285714,
//        multiplier * 1.07142857142857,  multiplier * 1.07142857142857,  multiplier * 1.07142857142857,  multiplier * 1.07142857142857,  multiplier * 1.07142857142857,
//        multiplier * 1.42857142857143,  multiplier * 1.42857142857143,  multiplier * 1.42857142857143,  multiplier * 1.42857142857143,  multiplier * 1.42857142857143
    };
    ...
    for (int j = 0; j < n_observations; ++j)
        std::cout << "K " << market_parameters.K[j] << " T " << market_parameters.T[j] << " jacobian " << jacobian[m * j] << " " << jacobian[m * j + 1] << " " << jacobian[m * j + 2]
                  << " " << jacobian[m * j + 3] << " " << jacobian[m * j + 4] << " " << std::endl;
    ...
    dlevmar_der(GetHestonPrice, GetHestonJacobian, p, x, m, n_observations, 200, opts, info, NULL, NULL, (void*) &market_parameters);
...
// K 0.9371 T 0.119048 jacobian 0.000134411 0.0280474 0.00330929 -0.00115206 0.151462
// K 0.9956 T 0.119048 jacobian 0.000226969 0.0375877 -0.000662676 -6.89813e-05 0.198523
// K 1.0427 T 0.119048 jacobian 0.000258176 0.0369337 -0.00438618 0.00114067 0.191592
// K 1.2287 T 0.119048 jacobian 3.47214e-05 0.00340939 -0.00150701 0.000600593 0.0164916
// K 1.3939 T 0.119048 jacobian 1.55588e-07 1.31452e-05 -7.96746e-06 4.44651e-06 6.03701e-05
