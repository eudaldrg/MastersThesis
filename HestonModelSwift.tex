...
In option pricing, one can denote by $\{S_t\}_{t=0}^T$ the underlying price process and model it with an exponential L\'evy process. For a given strike $K$, one can consider the log-transformed process $X_t := log(U_t / K)$, which will follow another L\'evy process with an extra drift. Let the state variables be $x := log(S_t / K)$ and $y := log(U_T / K)$. In order to solve equation \ref{eq:option_valuation} one needs to evaluate $f(y|x)$, which is not always available in analytic form, but formula \ref{levy_khin} can be used to obtain the form of its Fourier transform at any strike: 
...
Heston also provided an analytic equation to obtain the price of European options in terms of the characteristic function of the stochastic distribution of the underlying asset. Below is its expression for a Call option:
...
\end{aligned}
\end{equation}
. The proposed expression of the characteristic function is
...
\section{Characteristic function and Gradient Calculation}

Equation \ref{eq:char_heston}
...
The model is calibrated by using a gradient descent method as in \cite{cui17}, even though there is no guarantee the function is convex, so we might hit a local minimum. 

We will start from a set of parameters to price several options and we will try to recover the initial parameters after adding some random noise to the obtained prices.

Then we will use real market data and we will calibrate the model starting from several different initial guesses and check whether we obtain the same result.
