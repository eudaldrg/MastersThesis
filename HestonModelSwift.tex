...
\begin{equation}
\prod_{j=1}^{J} \cos \left(\frac{\pi x}{2^{j}}\right)=\frac{1}{2^{J-1}} \sum_{j=1}^{2^{J-1}} \cos \left(\frac{2 j-1}{2^{J}} \pi x\right)
\end{equation}
obtaining the first expression for the vieta coefficients $c_{m,k}^v$
\begin{equation}
c_{m,k} \approx c_{m, k}^{v}= \frac{2^{m / 2}}{2^{J-1}} \sum_{j=1}^{2^{J-1}} \int_{\mathbb{R}} f(x) \cos \left(\frac{2 j-1}{2^{J}} \pi\left(2^{m} x-k\right)\right) d x
\end{equation}
Finally, $f(x)$ would be replaced in the integral by defining $c_{m,k}^{v} = Re \left( c_{m,k} \right)$, where
\begin{equation}
 Re(\int_{\R} Re \left( f(x) e^{- uix} \right) dx = Re \left( \hat{f}(u) \right)
\end{equation}
Ending up with

This expression was revisited in \cite{mar17} as a particular case of expressing the sinc as 
...
The overall approximation can be summarized, as in \cite{mar17}, by a set of consecutive approximation steps. The error related to each of these approximations will be analyzed in section \ref{subsec:error_price}.
